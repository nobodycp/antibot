...
def telegram_backup_settings_view(request):
    ...
    if request.method == "POST":
        form = TelegramBackupSettingsForm(request.POST)
        if form.is_valid():
            cd = form.cleaned_data
            settings_obj.bot_token = cd["bot_token"]
            settings_obj.chat_id = cd["chat_id"]
            settings_obj.is_enabled = cd["is_enabled"]
            settings_obj.backup_database = cd["backup_database"]
            settings_obj.backup_media = cd["backup_media"]
            settings_obj.interval_days = cd["interval_days"]
        else:
...
def telegram_test_backup(request):
    ...
    early = redirect_if_telegram_unconfigured(request, settings_obj)
    if early:
        return early
